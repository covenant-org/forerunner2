...
FtpBridgePD::FtpBridgePD(const Core::ArgumentParser& args)
    : Core::Vertex(args),
      _mavsdk(
          mavsdk::Mavsdk::Configuration{mavsdk::ComponentType::GroundStation}) {
  this->_people_image_sub = nullptr;
  ...
  this->_ftp_config_sub = this->create_subscriber<KeyValue>(
      "config/ftp",
      std::bind(&FtpBridgePD::config_cb, this, std::placeholders::_1));
}

void FtpBridgePD::poll_files() {
        ...
        [=](mavsdk::Ftp::Result res, mavsdk::Ftp::ProgressData progress) {
          switch (res) {
            ...
            case mavsdk::Ftp::Result::Success: {
              ...
              this->_people_image_pub->_dangerously_raw_send(input.c_str(), total_bytes);
...
void FtpBridgePD::config_cb(const Core::IncomingMessage<KeyValue>& msg) {
  ...
  if (this->_people_image_sub == nullptr) {
    this->_logger.debug("Created people_image subscriber");
    this->_people_image_sub = this->create_subscriber<ImageData>(
        "people_image", std::bind(&FtpBridgePD::people_image_cb, this, std::placeholders::_1));
  }
}

void FtpBridgePD::people_image_cb(const Core::IncomingMessage<ImageData>& msg) {
...
//   cloudFile.close();
    auto imgData = msg.content.getData();

    std::ofstream file("/tmp/test_image.jpg", std::ios::binary);
    file.write(reinterpret_cast<const char*>(imgData.begin()), imgData.size());
    file.close();
    std::cout << "Wrote test_image.jpg (" << imgData.size() << " bytes)" << std::endl;
    this->_logger.info("Wrote test_image.jpg (%zu bytes)", imgData.size());

}

void FtpBridgePD::run() {
